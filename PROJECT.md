...
- Optional **chrome dim until hover/focus**: in idle state (`.app-shell--chrome-dim:not(:hover):not(:focus-within)`), the app enters a **true notes-only view**, shell background/border/shadow/blur are disabled, header and editor chrome are removed from layout, and task metadata controls (timestamp/delete) are hidden so only note text and checkboxes remain visible. On hover/focus, full chrome returns.
