...
- Optional **chrome dim until hover/focus**: uses a stabilized renderer state (`app-shell--chrome-active` / `app-shell--chrome-idle`) driven primarily by pointer enter/leave (plus focus activation) rather than blur-based deactivation, so moving within the notes area does not flap between windowless and full chrome. In idle mode, shell visuals are disabled and header/editor chrome are removed from layout.
