...
- Optional **chrome dim until hover/focus**: uses a stabilized renderer state (`app-shell--chrome-active` / `app-shell--chrome-idle`) driven primarily by pointer enter/leave (plus focus activation) rather than blur-based deactivation, so moving within the notes area does not flap between windowless and full chrome. In idle mode, shell visuals are disabled; the title bar is hidden in a layout-stable way (invisible, non-interactive, still occupying space) to avoid boundary flicker near the top edge.
