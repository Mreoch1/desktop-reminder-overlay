...
- Optional **chrome dim until hover/focus**: uses a stabilized renderer state (`app-shell--chrome-active` / `app-shell--chrome-idle`) driven primarily by cursor-in-shell state (plus settings open), explicitly ignoring lingering editor focus so the overlay returns to notes-only once pointer intent ends. Pointer activation includes a small bounds slop and short leave grace to prevent top-edge/header flicker.
- Additional guard: root `pointerenter`/`pointerleave` explicitly force intent on/off so leaving the window reliably returns to notes-only without requiring a click outside.
