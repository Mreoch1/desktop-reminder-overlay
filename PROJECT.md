...
- Optional **chrome dim until hover/focus**: uses a stabilized renderer state (`app-shell--chrome-active` / `app-shell--chrome-idle`) driven by global cursor position against shell bounds and real in-app interaction (`:focus-within` / settings open), not bare window focus. Result: idle is truly notes-only on desktop, and chrome appears only on hover or active interaction.
