...
- Optional **chrome dim until hover/focus**: uses a stabilized renderer state (`app-shell--chrome-active` / `app-shell--chrome-idle`) instead of raw CSS `:hover` for the major mode switch, with a short leave delay to avoid flicker when transitioning between notes-only and full chrome. In idle mode, shell visuals are disabled and header/editor chrome are removed from layout.
