...
- Optional **chrome dim until hover/focus**: uses a stabilized renderer state (`app-shell--chrome-active` / `app-shell--chrome-idle`) driven by global cursor position against the shell bounds plus window focus, instead of relying on header-region hover enter/leave events. This avoids frameless drag-region flicker at the top edge while keeping chrome visible when the app is active or the cursor is over it.
